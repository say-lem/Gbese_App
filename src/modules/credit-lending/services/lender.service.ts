...
export default class LenderService {
	...
	static async rejectLoanRequest(
		...
		// Create a new loan offer
		const updatedLoanOfferStatus = await LoanRepository.updateLoanOfferStatus(
			loanOffer.loanOfferId.toString(),
			"open",
			session
		);

        if (!updatedLoanOfferStatus) {
            throw new ApiError("Failed to approve loan offer", 400);
        }

		const updatedLoanOfferRequestId = await LoanRepository.updateLoanOfferLoanRequestId(
			loanOffer.loanOfferId.toString(),
			null,
			session
		)

		if (!updatedLoanOfferRequestId) {
			throw new ApiError("Error rejecting Loan request. try again", 400);
		}
